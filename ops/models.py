...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
        ...
        self.step = step
        if step == 0:
            raise ValueError("Grafting step not initialized.")
        self.teacher_network = True if step == -1 else False
        ...
        self.new_length = 3 if step < 2 else 1
    ...
    # def switch_tarining_status(self):
    #     self.isTraining = not self.isTraining

    def _prepare_tsn(self, num_class):
        if self.step == 1 or self.teacher_network:
            return
        else:
            feature_dim = getattr(self.base_model.p2, self.base_model.p2.last_layer_name).in_features

        if self.dropout == 0:
            setattr(self.base_model.p2, self.base_model.p2.last_layer_name, nn.Linear(feature_dim, num_class))
            self.new_fc = None
        else:
            setattr(self.base_model.p2, self.base_model.p2.last_layer_name, nn.Dropout(p=self.dropout))
            self.new_fc = nn.Linear(feature_dim, num_class)

        std = 0.001
        if self.new_fc is None:
            normal_(getattr(self.base_model.p2, self.base_model.p2.last_layer_name).weight, 0, std)
            constant_(getattr(self.base_model.p2, self.base_model.p2.last_layer_name).bias, 0)
        else:
    ...
    def forward(self, input):
        ...
        if self.teacher_network or self.step == 1:
            motion = self.base_model(input)
            return motion
        else:
