...
def main():
    ...
    if args.resume:
        ...
        init_ckpt_path = os.path.join(args.root_model, args.store_name, 'ckpt_best_step1.pth')
        init_checkpoint = torch.load(init_ckpt_path)
        sd = init_checkpoint['state_dict']
        model_dict = model.state_dict()
        replace_dict = {}
        for k, v in sd.items():
            if k.startswith('module.base_model.'):
                tmp_k = k.replace('module.base_model.', 'module.base_model.p1.')
                if tmp_k in model_dict:
                    replace_dict[tmp_k] = v
                    # print(tmp_k)

        logger.info(("loaded checkpoint for from step 1."))

        t_ckpt_path = os.path.join(args.root_model, 'Teacher__' + args.dataset + '/ckpt_best.pth')
        t_checkpoint = torch.load(t_ckpt_path)
        sd = t_checkpoint['state_dict']
        for k, v in sd.items():
            if k.startswith('module.base_model.'):
                tmp_k = k.replace('module.base_model.', 'module.base_model.p2.')
                if tmp_k in model_dict:
                    replace_dict[tmp_k] = v
                    
        logger.info(("loaded checkpoint for from Teacher."))
